...
UNITY_REPO=~/Unity/Hub/Editor
UNITY_VARIANT=linux64_withgfx_nondevelopment_mono
UNITY_ENGINE=Editor/Data/PlaybackEngines/LinuxStandaloneSupport/Variations/$UNITY_VARIANT
...
stat $UNITY_PATH/$UNITY_ENGINE >/dev/null
if [ ! $? -eq 0 ]; then
  ...
  tar -xf $UNITY_PATH/Unity.tar.xz -C $UNITY_PATH $UNITY_ENGINE
fi

cp -r $UNITY_PATH/$UNITY_ENGINE/* "$DATA_DIR/../"
